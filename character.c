...
int charge_minotaur(int *y, int *x, int player_y, int player_x, char charge_direction) {
    ...
    int newy=*y;
    int newx=*x;
    int movement=move_character(y, x, charge_direction, MINOTAUR);
    if (movement==MOVED_WALL) {
        ...
        map[newy*width+newx] = MINOTAUR;
        *y=newy;
        *x=newx;
    }
    movement=move_character(y, x, charge_direction, MINOTAUR);
